...
def plotdist3D(arr, cent):
    ...
    ax.set_zlabel('c')
    ax.set_ylim()
    ax.set_title('Distribution of parameter values of pow4')
    plt.show()
def flatten(arr):
    arrnew = []
    ...
    def k_mean_centroids(self):
        ...
        for m in self.model_ids:
            ...
            if rank == 0:
                ...
                with open('james1.p', 'rb') as file:
                    arr1 = pickle.load(file)
                df_performances = pd.read_csv("database-accuracy.csv")
                df_performances_one_dataset = df_performances.query(
                    '(openmlid ==  380|openmlid ==  346|openmlid ==  44|openmlid ==  3|openmlid ==  6| openmlid ==  1468|openmlid ==  41142)==False')

                cf = CurveFitting(df_performances_one_dataset, model_ids=[m],
                                  extrapolation_n=300, training_model_n=12, only_full_size=True, initailparams=arr1)
            ...
            if rank == 0:
                ...
                elif len(dff[dff['MSE_trn'] <= 0.025]) >15:
                    dff = dff[dff['MSE_trn'] <= 0.025]
                elif len(dff[dff['MSE_trn'] <= 0.25]) >15:
                    dff = dff[dff['MSE_trn'] <= 0.25]
                else:
                    dff = dff
                ...
                #plotdist2D(djj, ((z[0]*( djj[0]/whitened[0]))))
                #plotdist2D(djj, ((z[0]*( djj[0]/whitened[0]))))
        if rank == 0:
            ...
            for learner, df_learner in df_dataset.groupby("learner"):
                if learner in arr1[rank]:
                    ...
                    for i in range(0, len(self.model_ids)):
                        ...
                        if self.only_full_size & (len(mean_scores)>4):
                            offset = len(sizes)-1
                            ...
                            rows.append(
                                [openmlid, learner, np.array(sizes)[offset - 1], m.predict(), self.model_ids[i],
                                 ...
                                 0])
                        elif (len(mean_scores)>4)&(len(sizes)>4):
...
if __name__ == "__main__":
    ...
    rank = comm.Get_rank()
    cf = None
    #modeldd = ['pow2', 'log2', 'pow3', 'exp3', 'pow4', 'exp4']
    modeldd = ['pow2', 'log2', 'pow3', 'exp3', 'pow4', 'exp4']
    ...
    if False:
        ...
        if rank == 0:
            ...
            df_performances = pd.read_csv("database-accuracy.csv")
            df_performances_one_dataset = df_performances.query(
                   '(openmlid ==  44|openmlid ==  3|openmlid ==  6| openmlid ==  1468|openmlid ==  41142)')


            #df_performances_one_dataset = df_performances
            #cf = CurveFitting(df_performances_one_dataset, model_ids=None, extrapolation_n=300, training_model_n=25
                #              )#, initailparams=arr)

            cf = CurveFitting(df_performances_one_dataset, model_ids=['log2', 'pow4'], extrapolation_n=300, training_model_n=12, initailparams=arr)
            #cf = CurveFitting(df_performances_one_dataset, model_ids=[ 'pow4'], extrapolation_n=300, training_model_n=12, initailparams=arr)
            #cf = CurveFitting(df_performances_one_dataset, model_ids=['pow4'],
              #    extrapolation_n=300, training_model_n=12)
            print('computing extrapolations...')
    ...
    if False:
        for mm in[ 'log2','pow2', 'pow3', 'exp3', 'pow4', 'exp4']:
            cf = None
            if rank == 0:
                ...
                cf = CurveFitting(df_performances_one_dataset, model_ids=[mm], extrapolation_n=300, training_model_n=12, initailparams=arr)
                #cf = CurveFitting(df_performances_one_dataset,
                 #                 model_ids=[mm],
                #                  extrapolation_n=300, training_model_n=12)
                print('computing extrapolations...')
